...
对象、字段校验失败、`memory_id` 与文件名不符）在建局装载时即 fail-loud——API 报 500，CLI 报
参数错误，不会静默丢弃已有经验。
...
`save`，后写入的会覆盖先写入的（没有跨进程锁）；正常串行跑局不受影响。真人占用的座位其档案
（含 `memory_id`）不生效——该座位不会复盘也不会读取经验。多局累积经验没有一键命令，靠重复
用同一份 `agents.yaml`（同样的 `memory_id`）多次跑局自然积累。
