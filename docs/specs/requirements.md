...
`GAME_CREATED, PLAYER_JOINED, GAME_STARTED, ROLES_ASSIGNED(GM_ONLY), PHASE_CHANGED, GUARD_PROTECTED(ROLE_SELF), WOLF_KILL_PROPOSED(WOLVES), WOLF_KILL_DECIDED(GM_ONLY), WOLF_KILL_REVOTE(WOLVES), WITCH_SAVED(ROLE_SELF), WITCH_POISONED(ROLE_SELF), SEER_CHECKED(ROLE_SELF), NIGHT_RESOLVED(GM_ONLY), DEATH_ANNOUNCED(PUBLIC), SHERIFF_CANDIDACY(PUBLIC), SHERIFF_WITHDREW(PUBLIC), SHERIFF_VOTE_CAST(PUBLIC), SHERIFF_VOTE_STARTED(PUBLIC), SHERIFF_ELECTED(PUBLIC), SHERIFF_DIRECTION_SET(PUBLIC), SHERIFF_BADGE_LOST(PUBLIC), ELECTION_STAGE_CHANGED(PUBLIC), BADGE_PASSED(PUBLIC), PLAYER_SPOKE(PUBLIC), VOTE_CAST(PUBLIC 或按规则), VOTE_RESULT(PUBLIC), PLAYER_EXILED(PUBLIC), HUNTER_SHOT(PUBLIC), IDIOT_REVEALED(PUBLIC), LAST_WORDS(PUBLIC), GAME_OVER(PUBLIC)`。
...
**游标全部入流（issue #37）**：引擎不再有绕过事件流的状态写入——`step(state, action)` 后恒有 `reduce_all(state, events) == new_state`（全字段，含 `state_version`；`tests/test_replay_invariant.py` 逐步断言）。为此：`SHERIFF_VOTE_STARTED{candidates}` 在警长开票与 PK 开票时发出（候选集合与清空票箱，与 `VOTE_STARTED` 对称）；`ELECTION_STAGE_CHANGED{stage=withdraw}` 清空退水确认集合，退水期的 `SHERIFF_CANDIDACY`/`SHERIFF_WITHDREW` 由 reduce 累加确认；`ELECTION_STAGE_CHANGED{skip_day}` 记录竞选期自爆后跳过当天，`ROUND_STARTED`/`GAME_OVER` 清零；`PHASE_CHANGED{pending_hunter?, resume_token?}` 携带猎人待开枪座位与中断续接标记，reduce 在每个 `PHASE_CHANGED` 上原样写入 `resume_token`（`None` 即清零），`GAME_OVER` 亦清零。前端 TS reducer 按同一规则即可与后端逐事件等价；从事件日志重建的中途态（断线续局）与实时态一致。
